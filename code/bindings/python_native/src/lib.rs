...
#[pyfunction]
fn decode_to_json(data: &[u8]) -> PyResult<String> {
    decode_impl(data)
        .and_then(|decoded| {
            serde_json::to_string(&decoded).map_err(|err| format!("json serialization failed: {err}"))
        })
        .map_err(PyValueError::new_err)
}

#[pymodule]
fn zpe_ink_native(_py: Python<'_>, module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add_function(wrap_pyfunction!(version, module)?)?;
    module.add_function(wrap_pyfunction!(decode_to_json, module)?)?;
    Ok(())
}
...
fn decode_impl(input: &[u8]) -> Result<Decoded, String> {
    ...
    let mode = match input[6] {
        ...
        2 => "medium",
        3 => "sketch",
        value => return Err(format!("invalid mode code: {value}")),
    }
    .to_string();
    ...
    if hasher.finalize() != payload_crc {
        return Err("payload CRC mismatch".to_string());
    }

    let has_tilt = (flags & 0b010) != 0;
    let has_azimuth = (flags & 0b100) != 0;
    ...
    for _ in 0..stroke_count {
        ...
        for delta in x_deltas {
            x.push(x.last().copied().unwrap_or(0) + delta);
        }
        for delta in y_deltas {
            y.push(y.last().copied().unwrap_or(0) + delta);
        }
