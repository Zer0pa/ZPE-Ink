# ZPE-Ink Phase 03 Public Benchmarks

Public rows only. No proxy data is promoted here.

## Methodology

1. Artifact path: `proofs/reruns/phase3_public_benchmarks/phase3_public_benchmarks.json`
2. Baseline: raw float32 `x/y` payload, matching this bounded public-benchmark lane.
3. Fidelity rule: `exact` means decode output matched the source stroke arrays byte-for-byte at the integer channel level.
4. Registration-gated datasets stay blocked until the real corpus is acquired in-lane. No proxy values are substituted into this table.
